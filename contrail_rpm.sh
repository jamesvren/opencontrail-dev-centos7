...
yum install -y rpm-build scons git python-lxml wget gcc patch make cmake unzip flex bison gcc-c++ openssl-devel autoconf automake python-devel python-setuptools protobuf protobuf-devel protobuf-compiler net-snmp-python bzip2 boost-devel tbb-devel libcurl-devel libxml2-devel zlib-devel cppunit-devel cyrus-sasl-devel cyrus-sasl-lib openssl-devel cyrus-sasl python-sphinx.noarch kernel-devel libpcap-devel libpcap libnl-devel ibnl3-devel
...
#PRE_BUILD="utils/dkms/gen_build_info.sh VERSION $dkms_tree/vrouter/VERSION/build"
